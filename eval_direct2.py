...
def simple_cut(text, tags):
    ...
    if len(text)>0:
        ...
        try:
            ...
            _y_pred = sess.run([y_pred], feed_dict)
            _bilstm_output = sess.run([bilstm_output], feed_dict)[0]
            _y_pred = _y_pred[0][:text_len]  # padding填充的部分直接丢弃
            print(_y_pred, _y_pred.shape)
            print(_bilstm_output, _bilstm_output.shape)
            print(feed_dict[X_inputs])
            ...
            nodes = [dict(list(zip(config.tags[1:], each[1:]))) for each in _y_pred]
...
model_save_path = '/home/siyuan/data/model/hidden/bilstm.ckpt-1'  # 模型保存位置
...
def test_epoch(X_batch, y_batch):
    """Testing or valid."""
    fetches = [accuracy, cost, y_pred, softmax_w, softmax_b]
    _costs = 0.0
    _accs = 0.0

    X_batch = [word2id[i] for i in X_batch]
    print(X_batch)
    if len(X_batch) >= max_len:  # 长则弃掉
        X_batch = X_batch[:max_len]
    X_batch.extend([0]*(max_len-len(X_batch))) # 短则补全

    y_batch = [tag2id[i] for i in y_batch]
    yy = ""
    for i in y_batch:
        yy+=config.num_dict[i]

    print(yy)

    if len(y_batch) >= max_len:  # 长则弃掉
        y_batch = y_batch[:max_len]
    y_batch.extend([0]*(max_len-len(y_batch))) # 短则补全

    feed_dict = {X_inputs:np.array(X_batch).reshape(-1,100), y_inputs:np.array(y_batch).reshape(1,100), lr:1e-5, batch_size:1, keep_prob:1.0}
    _acc, _cost , _y_pred, _softmax_w, _softmax_b = sess.run(fetches, feed_dict)
    print("acc", _acc)
    print("cost", _cost)
    print("pred", _y_pred)
    print("softmax_w", _softmax_w)
    print("softmax_b", _softmax_b)
...
saver = tf.train.import_meta_graph('/home/siyuan/data/model/hidden/bilstm.ckpt-1.meta')
#saver.restore(sess,ckpt)
sess.run(tf.global_variables_initializer())
saver.restore(sess, "/home/siyuan/data/model/hidden/bilstm.ckpt-1")
#saver = tf.train.Saver(max_to_keep=10)
mymodel_file = "/home/siyuan/data/model/hidden/bilstm.ckpt-1"
# 通过张量的名称来获取张量
...
q = open("/home/siyuan/data/cut_char_without_marker_3.txt", "r")
...
for sentence in lines:
    if cnt_nm % 1000 == 1:
        saver.restore(sess, mymodel_file)
    ...
    pred = simple_cut(words, config.tags)
    test_epoch(words, tags)
